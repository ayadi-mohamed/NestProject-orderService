...
import { AddRequestApp, AddResponseApp, stateEnum, UpdateRequestApp, UpdateResponseApp, UpdateDataApp, RemoveRequestApp, RemoveResponseApp, GetRequestApp, GetResponseApp, SoftDltRequestApp, SoftDltResponseApp, RestoreDataApp, RestoreRequestApp, RestoreResponseApp, SearchRequestApp, SearchResponseApp } from './proto/application.pb';
...
export class ApplicationService implements OnModuleInit {
  ...
  public async removeApplication( payload: RemoveRequestApp): Promise<RemoveResponseApp> {
    const app = await this.repository.findOne(payload.idApp)
    if(! app){
      return { idApp: null, error: ['Application not found'], status: HttpStatus.NOT_FOUND };

    }else

    if( app.idUser == payload.idUser ){
        await this.repository.delete(payload.idApp);
        return { idApp: payload.idApp, error: null, status: HttpStatus.OK };
    }else{
      return { idApp: null, error: ['Application not found'], status: HttpStatus.NOT_FOUND };
